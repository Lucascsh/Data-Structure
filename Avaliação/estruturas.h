...
Arvore *inserirDoArquivo(Arvore **raiz) {
    char nomeArquivo[200];
    ifstream finder;
    //recebe o nome do arquivo
    cout << "Digite o nome do arquivo que contem os numeros: ";
    cin >> nomeArquivo;

    finder.open(nomeArquivo);
    ...
    while (getline(finder, linha)){
        //encontra o espaco em branco
        posicaoEspaco = linha.find(" ");
        numero = linha.substr(0, posicaoEspaco);
        //transmorma a substring em inteiro e insere na Arvore.
        *raiz = inserir(stoi(numero), *raiz);
    }
    finder.close();
    
}

bool maiorMenorMesmoNivel(int valor, Arvore *raiz){
    int qtdVezesEsquerda = contarVezes(valor, raiz->esq);//contar movimentos para a esquerda
    int qtdVezesDireita = contarVezes(valor, raiz->dir);//contar movimentos para a direita

    if (qtdVezesEsquerda = qtdVezesDireita){
        return true;
    }else{
        return false;
    }
...
Arvore *procurarValor(Arvore *raiz){
    int valor;
    ...
    if (raiz){
        if (valor==raiz->valor) {
            encontrado = true;
        }else if(valor < raiz->valor || valor > raiz->valor){
            encontrado = true;
        }else{
